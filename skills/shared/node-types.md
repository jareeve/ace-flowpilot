...
- Compute node → namespace prefix of `ComIbmCompute.msgnode`
  ...
  - In simple cases, the name of the Compute MODULE will often be the same as the name of the `.esql` file however, these are different concepts. Sometimes a single `.esql` file will contain multiple Compute MODULEs. In this circumstance it is the MODULE which should be used in the computeExpression and not the filename.  
